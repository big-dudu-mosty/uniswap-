...
const MASTER_CHEF_ADDRESS = import.meta.env.VITE_MASTER_CHEF_ADDRESS as `0x${string}` | undefined

// 添加调试日志
if (!MASTER_CHEF_ADDRESS) {
  console.warn('MASTER_CHEF_ADDRESS not configured in environment variables')
}
...
export const useFarming = () => {
  ...
  const deposit = async (poolId: number, amount: string, decimals: number = 18) => {
    if (!MASTER_CHEF_ADDRESS) {
      message.error('MasterChef 合约地址未配置')
      throw new Error('MASTER_CHEF_ADDRESS not configured')
    }
...
export const useReadFarming = (poolId: number, userAddress?: string) => {
  ...
  const { data: poolInfo } = useReadContract({
    ...
    args: [BigInt(poolId)],
    query: {
      enabled: isValidPoolId && !!MASTER_CHEF_ADDRESS,
    },
  })
  ...
  const { data: userInfo } = useReadContract({
    ...
    query: {
      enabled: isValidPoolId && !!userAddress && !!MASTER_CHEF_ADDRESS,
    },
  })
  ...
  const { data: lpBalance } = useReadContract({
    ...
    query: {
      enabled: !!poolInfo && !!userAddress && !!(poolInfo as any)?.[0],
    },
  })
  ...
  const { data: allowance } = useReadContract({
    ...
    query: {
      enabled: !!poolInfo && !!userAddress && !!(poolInfo as any)?.[0] && !!MASTER_CHEF_ADDRESS,
    },
  })
