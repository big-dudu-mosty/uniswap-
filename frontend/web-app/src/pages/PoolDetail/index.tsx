  ...
  const swapColumns = [
    {
      ...
      dataIndex: 'createdAt',
      key: 'createdAt',
      render: (text: string) => formatBeijingTime(text),
    },
    {
      title: '用户',
    ...
    },
    {
      ...
      render: (_: any, record: any) => {
        // 根据 tokenIn 地址判断使用哪个 decimals 和 symbol
        const isToken0In = record.tokenIn?.toLowerCase() === pool?.token0Address?.toLowerCase()
        const decimals = isToken0In ? (pool?.token0Decimals || 18) : (pool?.token1Decimals || 18)
        const symbol = isToken0In ? pool?.token0Symbol : pool?.token1Symbol
        const amount = formatUnits(BigInt(record.amountIn || '0'), decimals)
    ...
    },
    {
      ...
      key: 'tokenOut',
      render: (_: any, record: any) => {
        // 根据 tokenOut 地址判断使用哪个 decimals 和 symbol
        const isToken0Out = record.tokenOut?.toLowerCase() === pool?.token0Address?.toLowerCase()
        const decimals = isToken0Out ? (pool?.token0Decimals || 18) : (pool?.token1Decimals || 18)
        const symbol = isToken0Out ? pool?.token0Symbol : pool?.token1Symbol
        const amount = formatUnits(BigInt(record.amountOut || '0'), decimals)
        return `${parseFloat(amount).toFixed(4)} ${symbol}`
      },
    },
    {
