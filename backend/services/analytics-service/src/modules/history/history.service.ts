      ...
      const formattedData: SwapHistoryDto[] = data.map((item) => ({
        ...
        pool: item.pool
          ? {
              ...
              token0Address: item.pool.token0Address,
              token1Address: item.pool.token1Address,
              token0Decimals: item.pool.token0Decimals,
              token1Decimals: item.pool.token1Decimals,
            }
          : undefined,
